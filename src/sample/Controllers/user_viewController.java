...
public class user_viewController {
    ...
    void initialize() {
        ...
        zpButton.setOnAction(event -> {
            Salary_project_to_Sql.Make(client, 0.0, Controller.bank.getBik(), account_name[0][0],
                    String.valueOf(Controller.bank.getSalary_projects().size()));

            Salary_project salary_project = new Salary_project(String.valueOf(Controller.bank.getSalary_projects().size()),
                    Controller.bank.getBik(),client.getPassport(), 0.0, account_name[0][0], client.getCompany());
            Controller.bank.getSalary_projects().add(salary_project);
